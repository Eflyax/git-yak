<template>
	...
	>
		...
		>
			...
			>
				...
				<div v-if="current_operation !== null" class="flex items-center gap-1">
					...
					<btn
						v-if="current_operation.conflict"
		...
		<div v-else class="flex flex-col h-full">
			<div class="flex justify-end gap-1 flex-wrap mb-4">
				...
				<template v-else>
					<template v-if="current_commits.length === 1">
						<btn
							:disabled="
								current_branch_name === null && current_head === commit.hash
							"
						...
						<btn
							:disabled="current_head === commit.hash"
							@click="resetToCommit"
						>
					...
					<btn :disabled="working_tree_selected" @click="cherrypickCommits">
						...
						Cherry-pick
						{{
							current_commits.length > 1
								? `${current_commits.length} commits`
								: ""
						}}
					</btn>
					...
					<BranchModal
						v-if="show_branch_modal"
						:commit
						@close="show_branch_modal = false"
					/>
					<TagModal
						v-if="show_tag_modal"
						:commit
						@close="show_tag_modal = false"
					/>
				</template>
			</div>
			...
			<div
				class="min-h-14 overflow-auto shrink-0"
				:class="{ 'max-h-60': current_commits.length === 1 }"
			>
				...
				<div
					v-if="current_commits.length === 1 && commit.body"
					class="mt-4 mb-2 whitespace-pre-wrap"
				>
			...
			<template v-if="current_commits.length === 1">
				...
				<div
					v-for="name in commit.committer_email === commit.author_email
						? ['author']
						: ['author', 'committer']"
				>
					<div class="text-xs text-gray mt-1">{{ name }}:</div>
...
export default {
	...
	watch: {
		...
		async working_tree_files() {
			if (
				_.some(this.current_commits, { hash: "WORKING_TREE" }) &&
				this.current_commits.length <= 2
			) {
				await this.load();
			}
		},
	...
	methods: {
		async load() {
			...
			if (
				current_commits.length === 1 &&
				current_commits[0].hash === "WORKING_TREE"
			) {
				...
				const status = await this.repo.callGit(
					"diff",
					...hashes.reverse(),
					"--name-status",
					"-z",
			...
			if (this.selected_file !== null) {
				...
				const file = this.files[area].find(
					(file) => file.path >= this.selected_file.path,
				);
		...
		async cherrypickCommits() {
			try {
				await this.repo.callGit(
					"cherry-pick",
					..._.map(this.current_commits, "hash"),
				);
			} finally {
				await Promise.all([this.refreshHistory(), this.refreshStatus()]);
			}
		...
		async startRebase() {
			...
			if (this.selected_file !== null) {
				const parent_hash =
					commit.parents[0] ?? (await getEmptyRootHash(this.repo));
				await this.repo.callGit(
					"restore",
					"--source",
					parent_hash,
