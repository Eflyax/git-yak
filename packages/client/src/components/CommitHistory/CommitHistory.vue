	...
	<div class="commit-history">
		...
		>
			<icon name="mdi-magnify" />
			 <input
				...
				@input="resetSearch()"
				@keydown.enter.exact="
					search_index === null ? search() : changeSearchIndex(1)
				"
		...
		<div>
			<splitpanes
				...
				@resized="
					[commit_refs_column_size, commit_graph_column_size] = $_.map(
						$event,
						'size',
					)
				"
...
export default {
	...
	computed: {
		loaded_all() {
			return (
				...
				this.current_commit_limit > this.commits.length - 1
			);
		},
	},
	watch: {
		...
		selected_commits(new_value, old_value) {
			...
			if (
				...
				if (pos < state.start || pos + scroller.itemSize > state.end) {
					this.$refs.main_scroller.scrollToPosition(
						pos - (state.end - state.start) / 5,
					);
				}
			}
		},
	...
	methods: {
		...
		async loadHistory({ skip_references = false, limit } = {}) {
			if (!skip_references) {
				...
				if (this.selected_reference !== null) {
					const reference = _.find(this.references, {
						id: this.selected_reference.id,
					});
			...
			if (limit === undefined) {
				limit =
					this.search_index === null
						? this.commit_history_initial_limit
						: this.commit_history_search_limit;
		...
		async search() {
			...
			if (
				!this.loaded_all &&
				(this.commit_history_search_limit === null ||
					this.commits.length - 1 < this.commit_history_search_limit)
			) {
			...
			const regexes = this.search_query
				.split(/\s+/)
				.map(
					(term) =>
			...
			for (const [i, commit] of this.commits.entries()) {
				if (i > 0) {
					const values = [
						...
						"author_name",
						"committer_name",
					].map((attr) => commit[attr]);
					values.push(..._.map(commit.references, "name"));

					if (
						_.every(regexes, (regex) =>
							_.some(values, (value) => regex.test(value)),
						)
					) {
			...
			const preserved_index = found.indexOf(
				this.search_hashes[this.search_index],
			);
		...
		changeSearchIndex(delta) {
			if (this.search_hashes.length > 0) {
				this.setSearchIndex(
					(this.search_index + delta + this.search_hashes.length) %
						this.search_hashes.length,
				);
			}
		},
		...
		onScrollerUpdate() {
			...
			this.scrolled_to_bottom =
				state.end > (this.commits.length - 1) * scroller.itemSize;
		},
	},
};
</script>
