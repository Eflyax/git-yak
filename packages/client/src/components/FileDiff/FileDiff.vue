<template>
	<div class="h-full flex flex-col">
		<div class="flex items-center gap-2 p-2 pr-0">
			...
			<div v-if="unsaved_changes" title="Unsaved changes">*</div>
			<div class="grow" />
		...
		<div class="grow relative">
			...
			>
				<div v-if="show_no_changes_message">No changes</div>
...
class GlyphMarginWidget {
	...
	constructor({ diff_editor, lane, line_range_mapping, action, callback }) {
		...
		this.position = {
			lane,
			range: line_range_mapping.modified.toExclusiveRange(),
		};
							...
							[
								{
									...
									text: diff_editor._editors[source]
										.getModel()
										.getValueInRange(
											line_range_mapping[source].toExclusiveRange(),
										),
								},
							],
						);
	...
	computed: {
		...
		show_no_changes_message() {
			if (this.binary) {
				...
				return (
					this.diff_hunk_count === 0 &&
					this.collapse_unchanged_regions &&
	...
	methods: {
		onMountEditor(diff_editor) {
			...
			for (const name of ["original", "modified"]) {
				this.diff_editor._editors[name]
					.getModel()
					.setEOL(monaco_editor.EndOfLineSequence.LF);
			}
			...
			diff_editor.onDidUpdateDiff(async () => {
				...
				const actions = [
					...
					...(this.file.area === "committed"
						? []
						: [this.file.area === "unstaged" ? "stage" : "unstage"]),
				];
				for (const line_range_mapping of hunks) {
					widgets.push(
						...actions.map(
							(action, i) =>
								new GlyphMarginWidget({
									diff_editor,
									lane: i + 1,
									line_range_mapping,
									action,
			...
			if (this.side_by_side_view) {
				...
				diff_editor._sash.value._sash.onDidEnd(() => {
					const [a, b] = ["original", "modified"].map(
						(name) => this.diff_editor._editors[name]._domElement.clientWidth,
					);
		...
		getEditorContents() {
			return ["original", "modified"].map((name) =>
				this.diff_editor._editors[name].getValue(),
			);
		},
		async load() {
			...
			const loadOriginal = async () => {
				if (file.status === "A") {
					...
					if (rev === "EMPTY_ROOT") {
						...
						const file_path = ["R", "C"].includes(file.status)
							? file.old_path
							: file.path;
